...
-- Delete Booking records (bookings made by these users as guests)
DELETE FROM "Booking"
WHERE "guestId" IN (
...
\echo '✅ Deleted Booking records (as guest)'

-- Delete Booking records (bookings for cars owned by these users as hosts)
DELETE FROM "Booking"
WHERE "carId" IN (
...
\echo '✅ Deleted Booking records (as host)'

-- Delete Review records (reviews written by these users)
DELETE FROM "Review"
WHERE "reviewerId" IN (
  SELECT rp.id FROM "ReviewerProfile" rp
  WHERE rp."userId" IN (SELECT id FROM users_to_delete)
...
\echo '✅ Deleted Review records (as reviewer)'

-- Delete Review records (reviews for cars owned by these users)
DELETE FROM "Review"
WHERE "carId" IN (
  SELECT rc.id FROM "RentalCar" rc
  WHERE rc."hostId" IN (
...
\echo '✅ Deleted Review records (for their cars)'
