// app/fleet/analytics/components/BrowserDrilldown/BrowserThreatSummary.tsx
'use client'

import RiskBadge from '../shared/RiskBadge'

interface BrowserThreatSummaryProps {
  data: { avgRiskScore: number; maxRiskScore: number; vpnCount: number; proxyCount: number; torCount: number; hostingCount: number }
  totalViews: number
}

export default function BrowserThreatSummary({ data, totalViews }: BrowserThreatSummaryProps) {
    ...
    <div>
      ...
      {!hasThreats && data.maxRiskScore === 0 ? (
        <p className="text-sm text-green-600 flex items-center gap-2">
          ...
          Clean — no threats detected on this browser
        </p>
      ) : (
