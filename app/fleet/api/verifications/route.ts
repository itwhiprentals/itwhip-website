    ...
    if (filter === 'stripe_verified' || filter === 'all') {
      ...
      stripeGuests = profiles.map(p => ({
        ...
        stripe: {
          ...
          docFrontFileId: p.stripeDocFrontFileId,
          docBackFileId: p.stripeDocBackFileId,
          selfieFileId: p.stripeSelfieFileId,
          documentType: 'driving_license',
          issuingCountry: 'US',
        },
        // Fallback fields (from profile, not Stripe extraction)
        dateOfBirth: p.dateOfBirth,
        driverLicenseNumber: p.driverLicenseNumber,
        driverLicenseExpiry: p.driverLicenseExpiry,
