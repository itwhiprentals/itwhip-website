...
const CITY_FAQS = (cityName: string, carCount: number, minPrice: number) => [
  {
    ...
    answer: `Renting a car in ${cityName} with ItWhip is simple. Browse our ${carCount}+ available vehicles, select your dates, and book instantly. Choose delivery to your location or pick up from the host. All rentals include $1M liability coverage.`
  },
  {
    question: `What's the cheapest car rental in ${cityName}?`,
    answer: `Car rentals in ${cityName} start from $${minPrice}/day on ItWhip. We offer a range of vehicles from budget-friendly sedans to luxury SUVs. Book directly from local owners and save up to 35% compared to traditional rental companies.`
  },
  {
  ...
  },
  {
    ...
    answer: `Yes, all ItWhip rentals include $1M liability coverage. You can also add additional protection plans for comprehensive coverage. Guests can bring their own insurance for a 50% deposit discount.`
  },
  {
...
export async function generateMetadata({ 
  ...
  return {
    ...
    description: `Rent cars in ${cityName}, ${cityData.state} from $45/day. ${carCount} vehicles available ${carTypes}. Skip rental counters - book from local owners with free delivery. $1M insurance included.`,
    keywords: [
      `${cityName} car rental`,
      `rent a car ${cityName}`,
        ...
        <div className="mt-5 sm:mt-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {cityData.popularRoutes.map((route, i) => (
              <div key={i} className="px-2.5 py-2 sm:px-3 sm:py-2.5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg text-[10px] sm:text-xs text-amber-800 dark:text-amber-300 leading-snug">
                {route}
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {otherCities.map((city) => (
            <Link
              ...
              className="px-2.5 py-1.5 sm:px-3 sm:py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-amber-100 dark:hover:bg-amber-900/30 hover:text-amber-700 dark:hover:text-amber-400 transition-colors text-[10px] sm:text-xs font-medium"
            >
              {city} →
            </Link>
          ))}
