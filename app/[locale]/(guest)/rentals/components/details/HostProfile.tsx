...
export default function HostProfile({ host }: HostProfileProps) {
  ...
  const responseTimeText = processedHost.responseTime
    ? (processedHost.responseTime < 60
      ? `${processedHost.responseTime} ${t('min')}`
      : `${Math.floor(processedHost.responseTime / 60)} ${Math.floor(processedHost.responseTime / 60) === 1 ? t('hour') : t('hours')}`)
    : 'N/A'
          ...
          <div className="flex flex-col gap-2 text-sm text-gray-600 dark:text-gray-400">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                <span>{t('joined', { date: memberSinceFormatted })}</span>
              </div>
            </div>
      ...
      {processedHost.bio && (
        <div className="border-t border-gray-200 dark:border-gray-700 pt-5 mb-5">
          <h4 className="text-base font-medium text-gray-900 dark:text-white mb-3">
            {t('aboutHost', { name: processedHost.displayName })}
          </h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mt-4">
            {processedHost.languages && processedHost.languages.length > 0 && (
              <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <span>{t('speaks', { languages: processedHost.languages.join(', ') })}</span>
              </div>
            )}
            ...
            {processedHost.work && (
              <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <span>{t('worksIn', { work: processedHost.work })}</span>
              </div>
            )}
            {processedHost.totalReviews > 0 && (
              <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                <IoCheckmarkCircleOutline className="w-4 h-4 text-gray-400 flex-shrink-0" />
                <span>{t('reviewsCount', { count: processedHost.totalReviews })}</span>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400">
                  {t('messagingLockedDesc', { name: processedHost.displayName })}
                </p>
              </div>
            </div>
            ...
            <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
              <div className="flex gap-3 text-xs text-gray-500">
                <div className="flex items-center gap-1">
                  ...
                  <span>{t('respondsIn', { time: responseTimeText })}</span>
                </div>
                <div className="flex items-center gap-1">
                  <IoCheckmarkCircleOutline className="w-3 h-3" />
                  <span>
                    {t('responseRate', { rate: processedHost.responseRate ? Math.round(processedHost.responseRate) + '%' : 'N/A' })}
                  </span>
                </div>
              </div>
            </div>
