...
export default function GuestReviewSuccess({ review, onClose }: GuestReviewSuccessProps) {
  ...
  const shareOnTwitter = () => {
    const text = `Just had an amazing experience renting a ${review.car.year} ${review.car.make} ${review.car.model} on @ItWhip! ⭐️`
    const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(window.location.href)}`
    window.open(url, '_blank', 'width=600,height=400')
  }
  
  const shareViaEmail = () => {
    const subject = 'Check out my ItWhip rental experience!'
    const body = `I just rented a ${review.car.year} ${review.car.make} ${review.car.model} on ItWhip and had a great experience! Check it out: ${window.location.href}`
  ...
  return (
    <div className="text-center py-4">
      ...
      <p className="text-gray-600 dark:text-gray-400 mb-6">
        Your feedback helps other guests and helps {review.host.name} improve their service.
      </p>
        ...
        <div className="mb-6 animate-fade-in">
          <div className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
            ...
            <p className="text-xs text-amber-800 dark:text-amber-400 mb-3">
              Get 5% off your next booking with this code:
            </p>
      ...
      <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
        <p className="text-xs text-gray-500 dark:text-gray-400">
          Your review will be visible to other guests once our team verifies it (usually within a few minutes).
          The host may respond to your review to thank you or address any concerns.
        </p>
      </div>
    </div>
  )
