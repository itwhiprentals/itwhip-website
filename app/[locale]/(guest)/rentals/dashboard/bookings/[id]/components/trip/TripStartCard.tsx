...
export function TripStartCard({
  ...
  useEffect(() => {
    const updateTimers = () => {
      ...
      if (timeUntilTripEnd <= 0) {
      ...
      } else if (timeUntilPickup > 30 * 60 * 1000) {
        // More than 30 min before pickup — too early
        ...
        // Within 30 min of pickup — ready to start
        setTripStatus('ready')
        const minutes = Math.floor(timeUntilPickup / (1000 * 60))
        setTimeDisplay(minutes > 0 ? t('startsInMinutes', { minutes }) : t('readyForPickup'))
      } else if (Math.abs(timeUntilPickup) < 5 * 60 * 1000) {
        // At pickup time (within 5 min)
        setTripStatus('ready')
        ...
        <div className="p-4">
          ...
          {tripStatus === 'upcoming' && (
            <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 mb-4">
              ...
              <button
                disabled
                className="w-full mt-3 px-6 py-3 bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400 text-sm font-semibold rounded-lg cursor-not-allowed flex items-center justify-center gap-2"
              >
              ...
              <p className="text-[10px] text-gray-400 text-center mt-1.5">Available 30 minutes before pickup</p>
            </div>
          )}

          {tripStatus === 'ready' && (
            <div className="bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-lg p-3 mb-4">
              <h4 className="text-xs font-medium text-green-900 dark:text-green-200 mb-2">{t('checklistTitle')}</h4>
              <ul className="space-y-1 mb-3">
                {[t('checklistLicense'), t('checklistAgreement'), t('checklistRoute')].map((item, i) => (
                  <li key={i} className="flex items-center text-xs text-green-700 dark:text-green-300">
              ...
              <button
                ...
                disabled={isStarting}
                className="w-full px-6 py-3 bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white text-sm font-semibold rounded-lg transition-colors flex items-center justify-center gap-2"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
