...
import RentalSearchCard from '@/app/[locale]/(guest)/components/hero/RentalSearchWidget'
...
export default function SearchResultsClient({
  ...
  const carTypes = [
    { value: 'sedan', label: t('sedan'), icon: IoCarOutline },
    { value: 'suv', label: t('suv'), icon: IoCarOutline },
    { value: 'truck', label: t('truck'), icon: IoCarOutline },
    { value: 'van', label: t('van'), icon: IoCarOutline },
    { value: 'luxury', label: t('luxuryCar'), icon: IoCarOutline },
    { value: 'sports', label: t('sports'), icon: IoCarOutline },
    { value: 'convertible', label: t('convertible'), icon: IoCarOutline },
    { value: 'electric', label: t('electric'), icon: IoCarOutline }
  ]

  // Features for filter (not translated - these are brand/tech names)
  ...
  const deliveryOptions = [
    { value: 'airport', label: t('airportPickup'), icon: IoAirplaneOutline },
    { value: 'hotel', label: t('hotelDelivery'), icon: IoBusinessOutline },
    { value: 'home', label: t('homeDelivery'), icon: IoLocationOutline }
  ]
  ...
  const sortOptions = [
    ...
    { value: 'distance', label: t('distanceNearest') }
  ]
      ...
      <div className={`bg-white dark:bg-gray-800 sticky top-[60px] md:top-16 z-40 border-b border-gray-200 dark:border-gray-700 ${showMap ? 'shadow-md' : ''}`}>
        <div className={showMap ? 'w-full' : 'max-w-7xl mx-auto'}>
          ...
          <div className={`px-4 sm:px-6 lg:px-8 py-2 text-sm text-gray-600 dark:text-gray-400 ${showMap ? '' : 'border-b border-gray-100 dark:border-gray-700'}`}>
            {!isLoading && (
              <>
                <span className="font-medium text-gray-900 dark:text-white">{t('carsCount', { count: filteredCars.length })}</span>
                {searchMetadata && (
                  <>
                    <span className="mx-2">•</span>
                    <span className="text-green-600 dark:text-green-400">{t('fullyAvailable', { count: searchMetadata.fullyAvailable || 0 })}</span>
                    {searchMetadata.partiallyAvailable > 0 && (
                      <>
                        <span className="mx-2">•</span>
                        <span className="text-yellow-600 dark:text-yellow-400">{t('partialAvailability', { count: searchMetadata.partiallyAvailable })}</span>
                      </>
                    )}
                  </>
                )}
          ...
          <div className="overflow-x-auto">
            <div className="flex items-center gap-2 px-4 sm:px-6 lg:px-8 py-3 min-w-max">
              ...
              >
                ...
                <option value="available">{t('fullyAvailableFilter')}</option>
                <option value="partial">{t('partialAvailabilityFilter')}</option>
              </select>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="font-semibold text-amber-800 dark:text-amber-200">
                      {t('noTypeAvailable', { type: noResultsForType.charAt(0).toUpperCase() + noResultsForType.slice(1) })}
                    </h3>
                    <p className="text-sm text-amber-700 dark:text-amber-300 mt-0.5">
                      {t('noTypeAvailableDesc', { type: noResultsForType.toLowerCase(), city: searchedCity })}
                    </p>
                  </div>
                  <button
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                  {filters.carType.length > 0
                    ? t('noTypesCarsAvailable', { types: filters.carType.map(ct => ct.charAt(0).toUpperCase() + ct.slice(1)).join(', ') })
                    : t('noCarsFound')}
                </h3>
                <p className="text-gray-600 dark:text-gray-400 mb-6">
                  {filters.carType.length > 0
                    ? t('noTypesDesc', { types: filters.carType.map(ct => ct.toLowerCase()).join(' / '), city: searchedCity })
                ...
                >
                  {filters.carType.length > 0 ? t('browseAllCars') : t('clearFilters')}
                </button>
              </div>
            ) : (
                ...
                {filteredCarsInCity.length > 0 ? (
                  <div>
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                      ...
                      {t('carsInCity', { city: searchedCity })}
                      <span className="text-sm font-normal text-gray-500 dark:text-gray-400">
                        {t('available', { count: filteredCarsInCity.length })}
                      </span>
                    </h2>
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
                  ...
                  <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-6 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                      {t('noCarsInCity', { city: searchedCity })}
                    </h3>
                    <p className="text-gray-600 dark:text-gray-400 text-sm">
                      {filteredNearbyCars.length > 0
                        ? t('checkNearby', { count: filteredNearbyCars.length })
                ...
                {filteredNearbyCars.length > 0 && (
                  <div>
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                      ...
                      {filteredCarsInCity.length > 0 ? t('nearbyCarsOtherCities') : t('carsFromNearbyCities')}
                      <span className="text-sm font-normal text-gray-500 dark:text-gray-400">
                        {t('available', { count: filteredNearbyCars.length })}
                      </span>
                    </h2>
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
