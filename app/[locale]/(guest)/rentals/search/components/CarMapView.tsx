          ...
          <div className="p-3">
            ...
            <div className="flex items-center gap-2 mt-1 text-[11px] text-gray-600 dark:text-gray-400">
              {car.rating && car.rating.average > 0 ? (
                <span className="flex items-center gap-0.5">
                  ...
                  {tripCount > 0 && <span className="text-gray-500">{t('tripsCount', { count: tripCount })}</span>}
                </span>
              ) : tripCount > 0 ? (
                <span className="text-gray-500">{t('tripsCount', { count: tripCount })}</span>
              ) : (
            ...
            {car.host?.name && (
              <p className="text-[10px] text-gray-500 dark:text-gray-500 mt-1">
                {t('hostedBy', { name: car.host.name.split(' ')[0] })}
              </p>
            )}
              ...
              <div>
                ...
                {rentalDays > 1 && (
                  <p className="text-[10px] text-gray-500 dark:text-gray-400">
                    {t('totalForDays', { amount: totalPrice, days: rentalDays })}
                  </p>
                )}
              </div>
          ...
          <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-200 dark:border-gray-700">
            <div>
              <div className="flex items-baseline gap-1">
                ...
                <span className="text-sm text-gray-500 dark:text-gray-400">{t('perDay')}</span>
              </div>
              {rentalDays > 1 && (
  ...
  useEffect(() => {
    ...
    processedCars.forEach(car => {
      ...
      if (car.maxACCertified) {
        ...
        acIcon.title = t('maxACCertifiedTitle')
      ...
      if (car.nearAirport) {
        ...
        airportIcon.title = t('nearSkyHarborTitle')
        ...
        <div className="absolute inset-0 bg-white/90 dark:bg-gray-900/90 flex items-center justify-center">
          <div className="text-center max-w-xs">
            ...
            <p className="text-gray-900 dark:text-white font-medium mb-1">
              {t('findingCarsNearYou')}
            </p>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {t('tapMarkersToExplore')}
            </p>
          </div>
        </div>
      )}
        ...
        <div className="absolute top-4 left-4 right-4 flex items-start justify-between gap-2 pointer-events-none">
          ...
          <div className="px-3 py-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg pointer-events-auto">
            <span className="text-sm font-semibold text-gray-900 dark:text-white">
              {t('availableCarsCount', { count: processedCars.length })}
            </span>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 bg-white/95 dark:bg-gray-800/95 rounded-lg shadow-md text-[10px] text-gray-600 dark:text-gray-400 pointer-events-auto">
            ...
            <span>{t('approximateForHostSafety')}</span>
          </div>
        </div>
      )}
      ...
      {mapLoaded && (
        <div className="absolute bottom-20 left-4 hidden sm:flex flex-col gap-1.5 px-3 py-2 bg-white/95 dark:bg-gray-800/95 rounded-lg shadow-md text-[10px]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-600 dark:text-gray-400">{t('maxACPremiumCooling')}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[11px]">✈️</span>
            <span className="text-gray-600 dark:text-gray-400">{t('freeSkyHarborDelivery')}</span>
          </div>
        </div>
      )}
