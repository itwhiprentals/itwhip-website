...
export default function VehicleChangePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const t = useTranslations('VehicleChange')
  ...
  const handleAction = async (action: 'accept' | 'decline') => {
    ...
    if (action === 'decline' && !confirm(t('declineConfirm'))) {
      return
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 p-4">
        <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{t('linkUnavailable')}</h1>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 p-4">
        <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
            {result.type === 'accept' ? t('vehicleChangeAccepted') : t('bookingCancelled')}
          </h1>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8 px-4">
      <div className="max-w-lg mx-auto">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('vehicleChange')}</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('bookingNumber', { code: booking.bookingCode })}</p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg px-4 py-3 mb-6">
          ...
          <span className="text-sm text-amber-800 dark:text-amber-200">
            {t.rich('offerExpires', { hours: hoursLeft, strong: (chunks) => <strong>{chunks}</strong> })}
          </span>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden mb-4">
          ...
          {originalCar && (
            <div className="p-5 border-b border-gray-200 dark:border-gray-700 opacity-60">
              <p className="text-xs font-semibold text-gray-400 uppercase mb-3">{t('originalVehicle')}</p>
          ...
          {/* New vehicle */}
          <div className="p-5">
            <p className="text-xs font-semibold text-green-600 uppercase mb-3">{t('newVehicle')}</p>
            <div className="flex items-center gap-4">
              {newCar.image ? (
