...
export default function CreditsPage() {
  ...
  const getActionLabel = (action: string, type: string) => {
    switch (action) {
      case 'ADD':
        return type === 'CREDIT' ? t('creditAdded') : t('bonusAdded')
      case 'USE':
        return t('appliedToBooking')
      case 'EXPIRE':
        return t('expired')
      case 'ADJUST':
        return t('adjustment')
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-yellow-800 dark:text-yellow-300 mt-1">
                {t('creditsLockedDesc', { amount: totalCredits.toFixed(2) })}
              </p>
              <Link
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-800 dark:text-amber-300 mt-0.5">
                {t('bonusExpiringDesc', { amount: expiringAmount.toFixed(2) })}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-sm font-semibold text-green-900 dark:text-green-100">{t('creditsInfoTitle')}</h4>
        ...
        <div className="p-4 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-sm font-semibold text-purple-900 dark:text-purple-100">{t('bonusInfoTitle')}</h4>
              <p className="text-xs text-purple-800 dark:text-purple-300 mt-0.5">
                {t('bonusInfoDesc', { percent: Math.round(maxBonusPercentage * 100) })}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1">
              {(['all', 'credit', 'bonus'] as const).map((f) => {
                const filterLabels = { all: t('filterAll'), credit: t('filterCredit'), bonus: t('filterBonus') }
                return (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
        ...
        {filteredTransactions.length > 0 ? (
                    ...
                    {tx.expiresAt && (
                      <>
                        ...
                        <span className="text-amber-600 dark:text-amber-400">
                          {t('expiresDate', { date: new Date(tx.expiresAt).toLocaleDateString() })}
                        </span>
                      </>
                    )}
                  </div>
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {filter === 'all' ? t('noTransactions') : t('noFilterTransactions', { filter })}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
        <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-3">{t('howToEarn')}</h3>
        <div className="space-y-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-900 dark:text-white">{t('earnCreditsTitle')}</p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-900 dark:text-white">{t('earnBonusTitle')}</p>
              <p className="text-xs text-gray-600 dark:text-gray-400">
                {t('earnBonusDesc')}
              </p>
            </div>
