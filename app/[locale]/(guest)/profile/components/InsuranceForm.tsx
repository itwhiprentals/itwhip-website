...
const COVERAGE_KEYS = ['state_minimum', 'basic', 'standard', 'premium', 'custom'] as const
...
export default function InsuranceForm({ 
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, side: 'front' | 'back') => {
    ...
    if (!file.type.startsWith('image/')) {
      setErrors(prev => ({ 
        ...prev, 
        [side === 'front' ? 'cardFront' : 'cardBack']: t('uploadImageFile')
      }))
      return
    }
    ...
    if (file.size > 10 * 1024 * 1024) {
      setErrors(prev => ({ 
        ...prev, 
        [side === 'front' ? 'cardFront' : 'cardBack']: t('fileTooLarge')
      }))
      return
    }
  ...
  const validate = (): boolean => {
    ...
    if (!formData.provider.trim()) {
      newErrors.provider = t('providerRequired')
    }

    if (!formData.policyNumber.trim()) {
      newErrors.policyNumber = t('policyNumberRequired')
    }

    if (!formData.expiryDate) {
      newErrors.expiryDate = t('expiryRequired')
    } else {
      const expDate = new Date(formData.expiryDate)
      if (expDate < new Date()) {
        newErrors.expiryDate = t('insuranceExpired')
      }
    }

    if (!formData.coverageType) {
      newErrors.coverageType = t('coverageRequired')
    }

    if (formData.coverageType === 'custom' && !formData.customCoverage.trim()) {
      newErrors.customCoverage = t('customCoverageRequired')
    }

    if (mode === 'add') {
      if (!cardFront && !cardFrontPreview) {
        newErrors.cardFront = t('cardFrontRequired')
      }
      if (!cardBack && !cardBackPreview) {
        newErrors.cardBack = t('cardBackRequired')
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ submit: error instanceof Error ? error.message : t('submitFailed') })
    } finally {
      setLoading(false)
    }
  }
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
                {mode === 'add' ? t('titleAdd') : t('titleUpdate')}
              </h2>
              <p className="text-[10px] text-gray-600 dark:text-gray-400">
                {mode === 'add' ? t('subtitleAdd') : t('subtitleUpdate')}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto h-[calc(70vh-64px)] md:h-auto md:max-h-[calc(90vh-128px)] px-4 py-3">
          ...
          <div className="mb-3 p-2.5 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
            <p className="text-[10px] text-green-800 dark:text-green-200">
              <strong>{t('whyAddInsurance')}</strong> {t('whyAddInsuranceDesc')}
            </p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              >
                <option value="">{t('selectCoverageType')}</option>
            ...
            {formData.coverageType === 'custom' && (
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  placeholder={t('customCoveragePlaceholder')}
                  disabled={loading}
                  className={`w-full px-3 py-2 text-xs border rounded-lg focus:ring-2 focus:ring-green-500 dark:bg-gray-700 dark:text-white disabled:opacity-50 ${
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                {t('cardFrontLabel')} {mode === 'update' && <span className="text-[10px] text-gray-500">{t('optional')}</span>}
              </label>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                {t('cardBackLabel')} {mode === 'update' && <span className="text-[10px] text-gray-500">{t('optional')}</span>}
              </label>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                {t('additionalNotes')} <span className="text-[10px] text-gray-500">{t('optional')}</span>
              </label>
              <textarea
                name="notes"
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <span>{mode === 'add' ? t('addInsurance') : t('update')}</span>
              </>
            )}
          </button>
        </div>
