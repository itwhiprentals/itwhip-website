...
export default function ClaimDetailPage() {
  ...
  const handleSubmitResponse = async (e: React.FormEvent) => {
    ...
    if (responseText.trim().length < 100) {
      setSubmitError(t('responseMinLength'))
      return
    }
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">{t('loadingClaimDetails')}</p>
        </div>
      </div>
    )
  }
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                <h1 className="text-xl font-bold text-gray-900 dark:text-white">
                  {t('claimNumber', { id: claim.id.slice(-8).toUpperCase() })}
                </h1>
                {getStatusBadge()}
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {claim.isFiledByGuest ? t('filedByYou') : t('filedByHost')}{' '}
                {t('onDate', { date: new Date(claim.createdAt).toLocaleDateString('en-US', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
                }) })}
            ...
            <div className="flex items-start">
              ...
              <div className="flex-1">
                <h3 className={`font-semibold ${
                  ...
                  {claim.isUrgent ? t('urgentResponseRequired') : t('responseRequired')}
                </h3>
                <p className={`text-sm mt-1 ${
                  ...
                  {claim.hoursRemaining !== null && claim.hoursRemaining > 0
                    ? t('hoursRemaining', { hours: claim.hoursRemaining })
                    : t('submitResponseAsap')}
                  {claim.accountHoldApplied && ` ${t('accountOnHold')}`}
                </p>
              </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                <h3 className="font-semibold text-green-800 dark:text-green-300">
                  {t('responseSubmittedSuccessfully')}
                </h3>
                <p className="text-sm text-green-700 dark:text-green-400 mt-1">
                  {t('responseSubmittedDescription')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('type')}</p>
                ...
                {claim.incidentDate && (
                  <div>
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('incidentDate')}</p>
                    <p className="text-gray-900 dark:text-white">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        {t('charMinimum', { count: responseText.length })}
                      </p>
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        {t('evidencePhotosOptional')}
                      </label>
                      <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center">
                        <Upload className="w-8 h-8 text-gray-400 mx-auto mb-2" />
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          {t('uploadPhotosToSupport')}
                        </p>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-4 mb-4">
                  <div className="flex items-center text-green-700 dark:text-green-400 mb-2">
                    ...
                    <span className="font-medium">{t('responseSubmitted')}</span>
                  </div>
                  <p className="text-sm text-green-600 dark:text-green-500">
                    {t('submittedOn', { date: claim.guestResponseDate
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  ...
                  {claim.host.rating && (
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {t('ratingValue', { rating: claim.host.rating.toFixed(1) })}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600 dark:text-gray-400">{t('tier')}</span>
                    <span className="text-sm font-medium text-gray-900 dark:text-white">
                      {claim.policy.tier}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600 dark:text-gray-400">{t('deductible')}</span>
                    <span className="text-sm font-medium text-gray-900 dark:text-white">
                      ${claim.policy.deductible.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600 dark:text-gray-400">{t('liability')}</span>
                    <span className="text-sm font-medium text-gray-900 dark:text-white">
