...
export function ChoeAuto() {
  ...
  const devices = [
    { icon: IoDesktopOutline, labelKey: 'autoDevice1Label' as const, descKey: 'autoDevice1Desc' as const },
    { icon: IoPhonePortraitOutline, labelKey: 'autoDevice2Label' as const, descKey: 'autoDevice2Desc' as const },
    { icon: IoCarOutline, labelKey: 'autoDevice3Label' as const, descKey: 'autoDevice3Desc' as const }
  ]

  const voiceCommands = [
    {
      commandKey: 'autoVoiceCmd1' as const,
      typing: 'Choé, my cart',
      responseKey: 'autoVoiceResp1' as const
    },
    {
      commandKey: 'autoVoiceCmd2' as const,
      typing: 'Book number 1',
      responseKey: 'autoVoiceResp2' as const
    },
    {
      commandKey: 'autoVoiceCmd3' as const,
      typing: 'Confirm',
      responseKey: 'autoVoiceResp3' as const
    }
  ]

  const autoFeatureKeys = [
    'autoFeature1', 'autoFeature2', 'autoFeature3',
    'autoFeature4', 'autoFeature5', 'autoFeature6'
  ] as const
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`rounded-lg overflow-hidden border mb-8 shadow-lg transition-colors ${
          ...
          <div className={`px-5 py-4 border-b flex items-center gap-3 transition-colors ${
            ...
            >
              ...
              <span className="hidden sm:inline">{t('autoReplay')}</span>
            </button>
          </div>
          ...
          <div className="p-5 sm:p-6 space-y-5 min-h-[280px]">
              ...
              >
                ...
                {showResponse[index] && (
                  <div className={`ml-12 pl-3 border-l-2 border-[#e87040] animate-fadeIn ${
                    ...
                    <p className="text-xs">
                      <span className="text-[#27ca3f] font-medium">Choé:</span> {t(item.responseKey)}
                    </p>
                  </div>
                )}
              </div>
            ...
            {isComplete && (
              <div className="flex items-center gap-2 text-[#27ca3f] mt-4 animate-fadeIn">
                ...
                <span className="text-sm font-medium">{t('autoComplete')}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#1a1a1a] rounded-lg p-5 border border-[#27ca3f]/30 mb-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="font-bold text-gray-900 dark:text-white mb-1">{t('autoSafetyTitle')}</h3>
              <p className="text-sm text-gray-600 dark:text-[#a8a8a8]">
                {t.rich('autoSafetyDesc', { voiceOnly: (chunks) => <span className="text-[#27ca3f] font-medium">{t('autoSafetyVoiceOnly')}</span> })}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {autoFeatureKeys.map((featureKey) => (
            <div key={featureKey} className="flex items-center gap-2 text-sm text-gray-600 dark:text-[#a8a8a8]">
