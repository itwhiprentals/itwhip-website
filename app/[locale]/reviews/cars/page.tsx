...
function RatingBreakdown({
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-4 space-y-2">
      <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">{t('ratingBreakdown')}</h3>
      {ratings.map(star => {
...
function getFirstName(name: string | null | undefined, fallback: string = 'Guest'): string {
...
function getTypeLabel(type: string, t: (key: string, values?: Record<string, string | number | Date>) => string): string {
  const keyMap: Record<string, string> = {
    ...
    SPORTS: 'typeSports',
    LUXURY: 'typeLuxury',
    CONVERTIBLE: 'typeConvertible',
    VAN: 'typeVan',
    COUPE: 'typeCoupe',
    HATCHBACK: 'typeHatchback',
    EXOTIC: 'typeExotic'
  }
...
function getTimeAgo(date: Date | string, t: (key: string, values?: Record<string, string | number | Date>) => string): string {
  ...
  if (diffInDays < 30) { const count = Math.floor(diffInDays / 7); return t('weeksAgo', { count }) }
  if (diffInDays < 365) { const count = Math.floor(diffInDays / 30); return t('monthsAgo', { count }) }
  const count = Math.floor(diffInDays / 365); return t('yearsAgo', { count })
}
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description: t('reviewsCarsOgDescription'),
      url: getCanonicalUrl('/reviews/cars', locale),
      locale: getOgLocale(locale),
      type: 'website',
    },
    alternates: {
      canonical: getCanonicalUrl('/reviews/cars', locale),
      languages: getAlternateLanguages('/reviews/cars'),
    },
  }
}
...
export default async function CarReviewsPage({ searchParams }: PageProps) {
  ...
  const t = await getTranslations('ReviewsCars')
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
            ...
            <div className="text-center mb-6">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-100 dark:bg-amber-900/30 rounded-full text-amber-700 dark:text-amber-400 text-xs font-medium mb-3">
                ...
                {t('verifiedCarReviews')}
              </div>

              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 dark:text-white mb-2">
                {t('heroHeading', { count: totalCars })}
              </h1>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 text-center">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">{t('carReviewsStat')}</div>
              </div>
              <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 text-center">
                <div className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">{totalCars}+</div>
                <div className="text-xs text-gray-500 dark:text-gray-400">{t('ratedCars')}</div>
              </div>
              <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 text-center">
            ...
            <div className="max-w-md mx-auto mb-6">
              <RatingBreakdown distribution={ratingDistribution} total={reviewCount} t={t} />
            </div>

            {/* Google Reviews Badge with Title */}
            <div className="text-center mb-4">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">{t('googleBadgeText')}</p>
              <a
            ...
            <div className="max-w-xl mx-auto">
              <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg p-3 flex items-start gap-2">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400">
                  <span className="font-medium text-emerald-700 dark:text-emerald-400">{t('verifiedReviews')}</span> – {t('verifiedReviewsDescription')}
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="pt-8">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex gap-4 mb-6 border-b border-gray-200 dark:border-gray-700">
              ...
              >
                {t('tabCarReviews', { count: cars.length })}
              </Link>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-8 p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-600 dark:text-gray-400">{t('filterType')}</span>
                <div className="flex gap-1 flex-wrap">
                  ...
                  {['SUV', 'Sedan', 'Luxury', 'Sports', 'Truck'].map((type) => (
                    ...
                    >
                      {getTypeLabel(type.toUpperCase(), t)}
                    </Link>
                  ))}
                </div>
              </div>
              ...
              {/* Sort */}
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-600 dark:text-gray-400">{t('sortLabel')}</span>
                <div className="flex gap-1 flex-wrap">
                  {[
            ...
            {typeFilter && (
              <p className="mb-4 text-sm text-gray-600 dark:text-gray-400">
                {t('showingVehicles', { count: cars.length, type: getTypeLabel(typeFilter.toUpperCase(), t) })}
              </p>
            )}
          </div>
        </section>
                ...
                >
                  ...
                  <div className="relative aspect-[4/3] bg-gray-200 dark:bg-gray-700">
                    ...
                    <div className="absolute top-3 left-3 px-2 py-1 bg-white/90 dark:bg-gray-800/90 rounded-full flex items-center gap-1">
                      ...
                      <span className="text-xs text-gray-500">{t('tripsCount', { count: car.totalTrips })}</span>
                    </div>
                    {/* Type Badge */}
                    <div className="absolute top-3 right-3 px-2 py-1 bg-gray-900/70 text-white text-xs font-medium rounded">
                      {getTypeLabel(car.carType, t)}
                    </div>
                      ...
                      <div className="flex-1 text-right">
                        ...
                        {car.reviews[0]?.createdAt && (
                          <p className="text-[10px] text-gray-600 dark:text-gray-400 mt-0.5">
                            {getTimeAgo(car.reviews[0].createdAt, t)}
                          </p>
                        )}
                      </div>
                    </div>
                          ...
                          <div className="flex-shrink-0">
                            {car.reviews[0].reviewerProfile?.profilePhotoUrl ? (
                              <Image
                                ...
                                alt={getFirstName(car.reviews[0].reviewerProfile?.name, t('guest'))}
                                width={28}
                                height={28}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-xs font-medium text-gray-700 dark:text-gray-300">
                                {getFirstName(car.reviews[0].reviewerProfile?.name, t('guest'))}
                              </span>
                              {/* Dynamic stars */}
                              <div className="flex">
                                {[1, 2, 3, 4, 5].map((star) => (
