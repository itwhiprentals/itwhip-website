// app/partner/fleet/add/AddCarWizard.tsx
// Reusable Add Car Wizard — used standalone (page) or embedded (bottomsheet)
'use client'
...
interface AddCarWizardProps {
  mode?: 'standalone' | 'bottomsheet'  // Controls layout
  prefillDailyRate?: number            // From request.offeredRate
  defaultPublicListing?: boolean       // false for recruited hosts
  onComplete?: (carId: string) => void // Callback instead of router.push
  showListingToggle?: boolean          // Show "list publicly" toggle
}
...
export default function AddCarWizard({
  ...
  const [vehicleData, setVehicleData] = useState<VehicleData>(() => {
    if (prefillDailyRate && prefillDailyRate > 0) {
      return { ...INITIAL_VEHICLE_DATA, dailyRate: prefillDailyRate }
    }
  ...
  // Pre-fill daily rate if prop changes after mount
  useEffect(() => {
    if (prefillDailyRate && prefillDailyRate > 0 && vehicleData.dailyRate === 0) {
      setVehicleData(prev => ({ ...prev, dailyRate: prefillDailyRate }))
    }
  }, [prefillDailyRate])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        if (mode === 'bottomsheet' && onComplete) {
  ...
  // --- Render helpers for mode-conditional wrappers ---

  const isStandalone = mode === 'standalone'

  // Progress Steps indicator (shared between modes, but no sticky in bottomsheet)
  const progressSteps = (
    <div className={isStandalone ? 'max-w-4xl mx-auto px-4 pb-4' : 'px-4 pb-4'}>
  ...
  // Step content (identical in both modes)
  const stepContent = (
    <>
      {/* Step Errors */}
  ...
  if (mode === 'bottomsheet') {
    // Bottomsheet mode: no outer page wrapper, no header, no max-w constraint
    return (
  ...
  // Standalone mode: full page layout (same as original page.tsx)
  return (
