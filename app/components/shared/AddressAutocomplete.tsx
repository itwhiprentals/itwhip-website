// app/components/shared/AddressAutocomplete.tsx
'use client'
...
export function AddressAutocomplete({
  ...
  const searchAddresses = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      // Bias towards Arizona (Phoenix center)
      params.append('proximity', '-112.0740,33.4484')
  ...
  const parseAddress = (feature: MapboxFeature): AddressResult => {
    ...
    // If this is a place (city) result, use the main text as city
    if (!parsedCity && feature.id.startsWith('place')) {
      parsedCity = feature.text || ''
    }
  ...
  const handleSelect = (feature: MapboxFeature) => {
    ...
    // For city-only results, show the city name
    setQuery(parsed.streetAddress || parsed.city)
    setSuggestions([])
    setShowDropdown(false)
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <IoLocationOutline className="w-5 h-5 text-green-600 mt-0.5 flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-900 dark:text-white truncate">
                      {parsed.streetAddress || parsed.city || feature.text}
                    </p>
